    ...
    )
{
    ...
    weights.evaluation.max.iter <- ifelse(is.null(opt$weights.evaluation.epsilon), ifelse(method == 1, 10, 100), opt$weights.evaluation.epsilon)
    ...
    while(iter < max.iter)
    {
        ...
        if(method == 1)
            tmp <- evaluate.weights.1(x, p, eta, theta.fix, theta.var, weights.evaluation.epsilon, weights.evaluation.max.iter, support.epsilon)
        if(method == 2)
            tmp <- evaluate.weights.2(x, p, eta, theta.fix, theta.var, epsilon, jacob, weights.evaluation.epsilon, weights.evaluation.max.iter, support.epsilon)
        x <- tmp$x
        w <- tmp$w
